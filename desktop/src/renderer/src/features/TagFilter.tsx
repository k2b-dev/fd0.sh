...
export function TagFilter() {
  ...
  const selected = (tag: string) => vault.filters().tags.some((value) => value === tag);
  return (
    <>
      ...
      <Popover anchor={anchor} open={open()} onClose={() => setOpen(false)} role="dialog" label="Filter item tags" class="tag-filter">
        <div id={id}>
          ...
          <label class="tag-filter-option">
            <input type="checkbox" checked={vault.filters().untagged}
              onChange={(event) => vault.updateFilters({ untagged: event.currentTarget.checked })} />
            <span>Without tags</span>
          </label>
          <For each={visible()}>{(option) => (
            <label class="tag-filter-option">
              <input type="checkbox" checked={selected(option.tag)} onChange={() => vault.updateFilters({
                tags: selected(option.tag) ? vault.filters().tags.filter((tag) => tag !== option.tag) : [...vault.filters().tags, option.tag],
                untagged: false,
              })} />
